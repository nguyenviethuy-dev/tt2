  ...
  const handleAddToCart = () => {
    ...
    if (product) {
      dispatch({
        type: "ADD_ITEM",
        payload: {
          product, // Gửi product gốc không có selectedSize/Color
          quantity,
          size: selectedSize, // Gửi size và color riêng
          color: selectedColor
        }
      })
      router.push("/cart")
        ...
        <View className="space-y-2">
          ...
          <View className="flex-row flex-wrap gap-2">
            {sizes.map((size) => (
              ...
              >
                <Text className={`${selectedSize === size ? "text-primary-foreground" : "text-foreground"}`}>{size}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
          ...
          <View className="flex-row flex-wrap gap-2">
            {colors.map((color) => (
              ...
              >
                ...
                />
                <Text className={`ml-2 ${selectedColor === color ? "text-primary-foreground" : "text-foreground"}`}>{color}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
