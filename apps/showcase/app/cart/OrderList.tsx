...
interface OrderData {
  ...
  uid: string;
  createdAt: string;
  id?: string; // id không bắt buộc, sẽ tạo mới nếu thiếu
}
...
export default function OrderList() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const userOrders: OrderData[] = [];
      querySnapshot.forEach((doc) => {
        const orderData = doc.data() as Omit<OrderData, "id">;
        userOrders.push({ ...orderData, id: doc.id, uid: user.uid, createdAt: orderData.createdAt || new Date().toISOString() });
      });
      setOrders(userOrders);
      setLoading(false);
    } catch (error) {
  ...
  const handleViewReceipt = (order: OrderData) => {
    console.log("handleViewReceipt called with order before stringify:", order);
    router.push({
